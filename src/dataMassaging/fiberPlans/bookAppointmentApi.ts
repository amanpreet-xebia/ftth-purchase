...
const bookAppointmentApi = async (paymentDetails: any, orderId: any): Promise<responseType<any>> => {
  return trackPromise(
    fiberPlansService
      ...
      .then((response: any) => {
        ...
        if (message) {
          return { status: false, msg: data.messages || 'Failed to book appointment', data: {} };
        }
